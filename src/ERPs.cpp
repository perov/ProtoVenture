...
real ERP__Normal::GetSampledLoglikelihood(vector< shared_ptr<VentureValue> >& arguments,
  ...
  if (arguments.size() == 2) {
    ...
    double likelihood =
           gsl_ran_gaussian_pdf(ToVentureType<VentureReal>(sampled_value)->GetReal() -
                                  average,
                                sigma);
...
shared_ptr<VentureValue> ERP__Beta::Sampler(vector< shared_ptr<VentureValue> >& arguments) {
  real alpha;
  real beta;
  if (arguments.size() == 2) {
    VentureSmoothedCount::CheckMyData(arguments[0].get());
...
real ERP__UniformDiscrete::GetSampledLoglikelihood(vector< shared_ptr<VentureValue> >& arguments,
  ...
  if (arguments.size() == 2) {
    ...
    left_bound = arguments[0]->GetInteger();
    VentureCount::CheckMyData(arguments[1].get());
    right_bound = arguments[1]->GetInteger();
